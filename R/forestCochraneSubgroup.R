...
forest.CochraneSubgroup <- function(x, subgroup_label="Subgroup", ...) {
  ...
  if (inherits(x, "ReitsmaSubgroup") || inherits(x, "RutterGatsonisSubgroup")) {
    dt <- XP[,c("study","subgroup","TP","FP","FN","TN","Sensitivity (95%-CI)","Specificity (95%-CI)")]
    ...
    cc <- colnames(dt) 
    colnames(dt) <- c("Study",subgroup_label,cc[3:9],"Sensitivity (95%-CI)"," ","Specificity (95%-CI)")
  ...
  if(inherits(x,"HoyerAFT")){
    dt <- XP[, c("study",subgroup_label,"threshold", "TP","FP","FN","TN", "Sensitivity (95%-CI)", "Specificity (95%-CI)")]
    
    dt <- dt[order(dt$study,dt$subgroup,dt$threshold), ]
    dt$study <- as.character(dt$study)
    dt$study[duplicated(dt$study)] <- " "
    dt$study[duplicated(dt$subgroup)] <- " "
    ...
    dt$fspec  <- paste(rep(" ",18),collapse=" ")  
    cc <- colnames(dt) 
    colnames(dt) <- c("Study","Subgroup","Threshold",cc[4:10],"Sensitivity (95%-CI)"," ","Specificity (95%-CI)")
