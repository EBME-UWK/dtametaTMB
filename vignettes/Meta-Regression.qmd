...
Small numerical differences may arise from optimizer settings, numerical tolerances, and implementation details, but all benchmark estimates and standard errors were generally recovered to close agreement.
The Schuetz CT/MRI analysis reproduced the published subgroup parameter estimates, variance components, and graphical results closely. Likelihood-ratio tests led to identical substantive conclusions, although the LR statistics differed somewhat from the published values.
...
plot(schuetzreitsma4,predlevel=0,
     nudge_legend=-0.2,
     size="se",scale=0.0025,
     col=c("red","black"))
### add connecting lines to the plot
op <- par(mar = c(5, 4, 4, 10), pty="s")
draw <- schuetzreitsma4$data
for(st in unique(draw$study)) {
  tmp <- draw[draw$study == st, ]
...
schuetz5 <- fitRutterGatsonis(data=subset(schuetz2,test=="MRI"),
                       TP=TP,FP=FP,FN=FN,TN=TN,
                       study=study)                       
```
