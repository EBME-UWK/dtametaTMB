...
The package allows model parameters to be constrained to zero during estimation. This can be useful in meta-analysis of sparse data, when convergence is difficult, or when fitting simplified models as descriped in the Cochrane Handbook. For example,
...
```
fixed the between study random effects covariance at zero. Other options include set `constrain="sigma2_A"`, `"constrain=sigma2_B"`, or `"constrain=all"`. 
